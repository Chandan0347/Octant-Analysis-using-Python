...
os.chdir(r'C:\Users\Chandan\OneDrive\Documents\GitHub\2001CB21_2022\tut07')
...
# Function Defination
def octant_range_names(mod, size_a,path):
    ...
    # Creating dataframe of excel file
    ...
    # Except block if File not found or incorrect directory is opened
        ...
        exit()
# Function Defination
def octant_transition_count(mod,iter, size_a,path):
    # Creating dataframe from reading excel file
    df = pd.read_excel(path)
    ...
    df.to_excel(path, index=False)
# Function Defination
def octant_longest_subsequence_length_count_with_time_ranges(size_a, path):
    # Creating dataframe for reading excel file
    df= pd.read_excel(path)
    df["Octant"]=np.nan
    df.loc[0, "Octant"]="+1"
    df.loc[1, "Octant"]="-1"
...
# Function Defination
def add_borders(filepath,iter, total_count):
    ...
    range=ws['N1':f'AF{iter+2}']
    # For loop for creating border table
    for cell in range:
        for x in cell:
            x.border=border
    # Range for which border should be given
    range = ws[f'AC{iter+4}':f'AE{iter+12}']
    # For loop for creating border table
    for cell in range:
        for x in cell:
            x.border=border
    ...
    while n<iter:
        range = ws[f'AI{17+13*n}':f'AQ{25+13*n}']
        for cell in range:
            for x in cell:
                x.border=border
        n =n+1
    # Range for which border should be given
    range = ws[f'AS1':f'AU9']
    # For loop for creating border table
    for cell in range:
        for x in cell:
            x.border=border
    # Range for which border should be given
    range = ws[f'AW1':f'AY{total_count+17}']
    # For loop for creating border table
    for cell in range:
        for x in cell:
            x.border=border
    # Saving workbook to filepath
    wb.save(filepath)
# Function Defination
def highlight_cells(filepath,iter, total_count):
    # Opening workbook from path by using openpyxl
    wb= openpyxl.load_workbook(filepath)
    ws = wb['Sheet1']
...
def octant_analysis(mod,path):
    ...
    # Calling different functions for operations
    octant_range_names(mod,size_a,path)
    octant_transition_count(mod,iter, size_a,path)
    total_count = octant_longest_subsequence_length_count_with_time_ranges(size_a, path)
    create_empty_columns(path)
    # command for avoiding Bold header
    pandas.io.formats.excel.ExcelFormatter.header_style = None
    add_borders(path,iter, total_count)
    highlight_cells(path,iter, total_count)
...
file_list = glob.glob(path1+'\input\*.xlsx')
# Creating list of files in directory
dir_list = [path_leaf(path) for path in file_list]

print("For Changing mod Value, please go to line no. 30\n")
print(f"Please wait! Processing\n")
# Iterating over each file for writing programme in excel
for file in dir_list:
    ...
    try:
        ...
        original = f'input\{file}'
        target = f'output\{file_name}_octant_analysis_mod_{mod}.xlsx'
    ...
    # Except statement if error occured in try block like input file does not exist or Correct Directory is not open
        ...
        print(f"Correct input file name: {file}")
        exit()

    # Try block if output file do not created or exist
    try:
        # Created dataframe of output file using pandas library and read_excel() function 
        df= pd.read_excel(f'output\\{file_name}_octant_analysis_mod_{mod}.xlsx')
        size_a=df['U'].size
        # Calculating no. of iterations for mod ranges using ceil function of Math library
        try:
            iter = math.ceil(size_a/ mod)
        ...
        path = f"output\{file_name}_octant_analysis_mod_{mod}.xlsx"
        
    # Except block if Output file is not found
    except:
        print("Output file not found. Error! Stoping the programme")
        exit()
    # Function Call for octant_analysis()
    octant_analysis(mod,path) 

print("Your Files are ready! Please Check!")
